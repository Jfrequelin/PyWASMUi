...
def test_example_ui_with_selenium(case: dict[str, str], chrome_driver) -> None:
    by = pytest.importorskip("selenium.webdriver.common.by").By
    wait = pytest.importorskip("selenium.webdriver.support.ui").WebDriverWait(
        chrome_driver,
        20,
    )
    ...
    with _live_example_server(case) as live_server:
        ...
        initial = wait.until(ec.presence_of_element_located((by.ID, case["initial_id"])))
        assert case["initial_text"] in initial.text
